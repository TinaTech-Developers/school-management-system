  ...
  useEffect(() => {
    if (!searchTerm) {
      ...
      setFilteredConversations(
        conversations.filter((c) => c.user.name.toLowerCase().includes(term)),
      );
    }
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      const res = await fetch(
        `/api/student/messages?userId=${selectedUser._id}`,
      );
      const data = await res.json();
    ...
    <div className="w-full mx-auto">
      ...
      <div className="flex gap-6">
        ...
        <div className="w-1/4 bg-gray-100 rounded-lg shadow p-4">
          <h2 className="font-bold text-gray-700 mb-2">Teachers</h2>
        ...
        <div className="flex-1 flex flex-col bg-white rounded-lg shadow p-4">
          <p className="font-bold text-gray-700 mb-2 border-b pb-2">
            Chatting with{" "}
            {selectedUser ? selectedUser.name : "Select a teacher to chat"}
          </p>
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex items-center justify-end gap-1 mt-1 text-[10px] opacity-80">
                      ...
                      {isMine && (
                        <span
                          className={`${m.read ? "text-green-500 text-sm" : "text-gray-300"}`}
                        >
                          {m.read ? "✔✔" : "✔"}
                        </span>
                      )}
